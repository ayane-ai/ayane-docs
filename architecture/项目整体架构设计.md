...
- API、WebSocket、Session、Identity、Memory、Agent Runtime、World Model、账号鉴权与语音识别合成都属于 `ayane-agent-service`。
...
Phase 1 不建立独立仓库：上述契约由 `ayane-agent-service` 的 `contracts` 模块承载，随服务端一起演进；满足 [Contracts 架构设计](Contracts架构设计.md) 第 7 节的触发条件后再抽出为独立仓库并启用 SemVer 发版。
...
- 音频采集与播放（Phase 1）；视频采集属 Phase 2
- 麦克风权限（Phase 1）与摄像头权限（Phase 2）
...
- 云端 API Key 不进入任何客户端。
- 音频帧与动作事件复用同一连接：控制帧为 JSON，音频帧为二进制；动作通过回复标识与音频关联。
