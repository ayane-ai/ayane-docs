...
本文档以 [产品愿景与总纲](../product/产品愿景与总纲.md) 为产品目标，固定第一阶段及后续阶段的工程边界。
...
flowchart TB
    ...
    subgraph Contract["跨仓库契约（Phase 1 内嵌于 ayane-agent-service）"]
        direction TB
    ...
    subgraph Body["Unity 独立仓库：ayane-embodiment"]
        direction TB
...
- 用户输入、音频与视觉采集、权限、通知、本地缓存属于各平台客户端。
- `commonMain` / `app:shared` 负责共享 Session、状态、网络和业务模型。
- Desktop 是第一阶段的完整验证端。
- Android 和 iOS 负责各自平台入口与平台能力，不互相依赖。
- 各端通过自己的 Unity Bridge 引用 `ayane-embodiment` 发布的版本化产物，不引用 Unity 源码。

### 后端归属

- API、WebSocket、Session、Identity、Memory、Agent Runtime、World Model、账号鉴权、语音识别合成与视觉理解都属于 `ayane-agent-service`。
...
- 契约是 API、Event 和 Action Protocol 的唯一来源；Phase 1 由 `ayane-agent-service` 的 `contracts` 模块承载，独立仓库 `ayane-contracts` 延迟建立。
- 客户端和后端不自行定义互不兼容的消息格式；Unity 使用自己的 Embodiment API。
- KMP 客户端负责将 Agent Action Protocol 映射为 Unity Embodiment API。
- Phase 1 契约与 Service 同步演进；契约独立后，协议升级顺序为 Contracts → Service / Client。Unity Runtime 始终按自身 API 版本独立演进。
...
- 平台支持多用户，每个用户可拥有多个 Agent；Identity、Memory、State 按「用户 + Agent」维度隔离。
...
- Personality
- Memory
- Relationship（关系状态，归 Identity 与 Memory）
- Session
- 文字对话
- 云端模型调用
...
- 语音输入输出（客户端采集、播放与本地打断检测；识别与合成在服务端）
- 视觉（客户端采集与在场检测；服务端屏幕内容理解与人脸跟踪）
- 用户账号、登录鉴权与 Session
- Agent 归属登记与默认 Agent
...
```text
ayane-ai/
├── ayane-contracts（Phase 1 内嵌于 ayane-agent-service，暂不建立）
├── ayane-client
├── ayane-agent-service
├── ayane-embodiment
├── ayane-admin-web
...
Phase 1 不建立独立仓库：上述契约由 `ayane-agent-service` 的 `contracts` 模块承载，随服务端一起演进；满足 [Contracts 架构设计 §7 契约独立的触发条件](Contracts架构设计.md) 后再抽出为独立仓库并启用 SemVer 发版。
...
- Unity 发布产物的版本管理
- Agent 列表、绑定与切换（Phase 1 每用户一个默认 Agent；多 Agent 自助创建与跨 Agent 切换属 Phase 1 内延后项）
- 登录状态与 Token 生命周期
- 音频采集与播放、本地打断检测
- 屏幕与摄像头采集、降采样、变化检测与节流，以及端侧在场检测

该仓库不包含后端实现，不包含 Unity 工程，不直接调用 LLM。它只通过各端 Bridge 引用 Unity 仓库发布的版本化产物。

### 3.3 [ayane-agent-service](AgentService架构设计.md)

独立部署的 Agent 后端，使用 Kotlin + Ktor + Koog 构建，通过 Koin 进行轻量依赖注入、通过 Exposed 访问服务端数据库，负责：
...
### 3.4 [ayane-embodiment](Unity身体架构设计.md)
...
```text
contracts 模块（Phase 1 位于 ayane-agent-service，后独立为 ayane-contracts）
       ↑
       ├── ayane-agent-service
       ├── ayane-client
       └── ayane-admin-web
...
客户端      ✕ LLM API
客户端      ✕ Unity 工程源码
Unity       ✕ 契约（contracts 模块 / ayane-contracts）
...
- 音频采集与播放（Phase 1）
- 屏幕与摄像头采集、降采样、变化检测与节流（Phase 1）
- 端侧在场检测（Phase 1）
- 麦克风、屏幕捕获与摄像头权限（Phase 1；摄像头在用户授权后开启）
- Token 安全存储（系统凭据库 / Keychain / Keystore）
- 音频会话管理与本地打断检测
- 通知
- 本地存储
- 生命周期
...
### 6.2 Agent Action Protocol

协议由契约统一定义（Phase 1 为 `ayane-agent-service` 的 `contracts` 模块），至少包含协议版本、Session 标识、动作序列和动作参数。
...
- 云端 API Key 不进入任何客户端。
- 音频帧与视觉帧复用同一条连接：控制帧为 JSON，音频帧与视觉帧为二进制；动作通过回复标识与音频关联。
...
- LLM API
- 用户账号系统
- 契约（contracts 模块 / ayane-contracts）
...
```text
1. 用户账号、登录鉴权与 Agent 归属登记
2. Desktop 文字对话
3. Identity 数据结构
4. SQLite 基础 Memory
5. Session 和流式响应
6. Agent Action Protocol
7. Unity Avatar 接收并执行动作
8. Desktop 语音输入输出（含服务端语音层接入）
9. Desktop 视觉（采集与在场检测 + 服务端视觉层接入）
10. Android 客户端基础接入
11. iOS 客户端基础接入
12. 主动行为和后台调度
```
...
- Phase 1 契约随 `ayane-agent-service` 版本演进；契约独立为 `ayane-contracts` 后使用 SemVer。
- 契约独立后，协议变更必须先升级 Contracts。
...
升级顺序（契约独立后；Phase 1 契约与 ayane-agent-service 同步发布）：
...
- 契约模块（Phase 1，位于 `ayane-agent-service`）/ `ayane-contracts`（独立后）是 API、Event 和 Action Protocol 的唯一来源。
- `ayane-client` 负责将 Agent Action Protocol 映射为 Unity Embodiment API。
- `ayane-embodiment` 独立发布 Desktop、Android、iOS 产物。
- `ayane-client` 只通过平台 Bridge 和固定版本产物引用 Unity 功能。
- 客户端仓库不直接依赖 Unity 工程源码。
- Unity 仓库不依赖契约模块、`ayane-contracts`、Agent Service 或客户端源码。
- `ayane-admin-web` 独立于 KMP 客户端，并只访问 Admin API。
- Desktop 是 Phase 1 的主验证平台。
- 跨用户隔离可验证：使用他人的登录凭据访问其 Agent 必须被拒绝，且不读取任何记忆数据。
- 语音闭环可验证：不依赖键盘完成一次对话，并可被用户开口打断。
- 视觉闭环可验证：客户端上行的屏幕与摄像头帧能产出视觉摘要，断开视觉上行时端侧在场检测与文本对话仍可用。
- 模型路线是云端 OpenAI-compatible API。
- AI Identity 不绑定任何具体客户端或身体。
- 契约独立后，协议升级可以按 Contracts → Service / Client；Unity Runtime 按自身 Embodiment API 版本独立演进。
