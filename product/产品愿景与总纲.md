...
- Unity 只负责表现，不承载记忆
- 协议统一，各端独立演进
- 记忆、人格与关系数据属于用户：平台必须支持查看、导出与可验证的删除；语音识别与合成的音频属于音频留存，仅用于审计与回溯，用户级删除时级联清除，但不提供面向用户的查看与导出入口；服务端是权威副本，客户端只持有缓存。
...
| 屏幕内容理解 | Screen Content Understanding | 理解用户屏幕上的内容，Phase 1 基础视觉之一 | 屏幕理解 |
| 在场检测 | Presence Detection | 判断用户是否在设备前，Phase 1 基础视觉之一 | 存在检测、在位检测 |
| 人脸跟踪 | Face Tracking | 持续跟踪用户面部位置与朝向 | 人脸识别/跟踪、面部跟踪 |
