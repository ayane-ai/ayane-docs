...
本文档的唯一上位目标是 [凌音 AI 数字生命](../product/产品愿景与总纲.md)。所有范围、技术选型和实现顺序都必须服从该页面的产品愿景与架构原则；若出现冲突，以目标页面为准。
...
- **语音与视觉同构**：端侧只负责采集、节流与在场检测，识别、合成与视觉理解都在服务端完成，跨设备保持一致。
...
Phase 1 的视觉与语音同构：客户端只做屏幕与摄像头采集、降采样、变化检测与节流，并把在场检测留在本地；屏幕内容理解与人脸跟踪由服务端视觉层调用云端多模态模型完成，摄像头在用户授权后开启。视觉帧与音频帧复用同一条连接上行，原帧默认短期留存 7 天，只有视觉摘要进入感知事件与对话上下文。完整视觉理解（用户位置、姿态、手势、环境理解与图像摘要）留到 Phase 2。

视觉引擎沿用云端多模态理解端点（与文本模型同一供应商路径），Phase 1 不引入自托管视觉模型；自托管多模态候选的能力、许可证与版本信息待核实后再登记（当前无可引用来源）。
...
├── 客户端视觉：屏幕与摄像头采集、降采样、节流、端侧在场检测
├── 服务端视觉：云端多模态理解（屏幕内容理解、人脸跟踪），原帧短期留存
...
- 识别或合成不可用时退回文本，对话不中断。
- 视觉闭环：上行一帧屏幕或摄像头画面能产出视觉摘要；断开视觉上行时端侧在场检测与文本对话仍可用。
...
- 自研大模型
- 复杂向量数据库和完整长期记忆平台
- 完整视觉理解（用户位置、姿态、手势、环境理解与图像摘要）
- 自托管视觉模型（Phase 1 视觉引擎使用云端多模态端点，候选与许可证待核实）
